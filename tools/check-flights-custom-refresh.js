...
(async () => {
  ...
  // Immediately check that the spinner appeared in the flights summary
  const spinnerPresentImmediate = await page.evaluate(() => !!document.querySelector('#flights-summary .spinner'));
  ...
  try {
    ...
    console.error('Timed out waiting for _lastLoadFlights to update (120s)');
  }

  const lastLoad = await page.evaluate(() => window._lastLoadFlights || null);
  console.log('Last loadFlights run time =>', lastLoad);
