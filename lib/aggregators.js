...
async function computeSquawkTransitionsDataByTimeRange(s3, readBucket, startTime, endTime, source = 'read') {
    try {
        ...
        // Get all piaware log files (we'll filter by data timestamp, not file modification time)
        let minuteFiles = (allFiles || [])
            .filter(f => f.Key && f.Key.includes('piaware_aircraft_log'))
            .sort((a, b) => new Date(a.LastModified) - new Date(b.LastModified));

        console.log(`[squawk-debug] Found ${minuteFiles.length} total log files to process`);
        ...
        console.log(`[squawk-debug] Found ${transitions.length} total transitions, filtering by time range ${new Date(startTime).toISOString()} to ${new Date(endTime).toISOString()}`);
        
        // Filter transitions by their actual timestamp (not file modification time)
        const filteredTransitions = transitions.filter(t => t.timestamp >= startTime && t.timestamp <= endTime);
        
        console.log(`[squawk-debug] After time range filter: ${filteredTransitions.length} transitions`);
        filteredTransitions.sort((a, b) => b.timestamp - a.timestamp);
