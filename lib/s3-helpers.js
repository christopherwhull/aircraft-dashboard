...
async function listS3Files(s3, bucketName, prefix = '', maxKeys = 1000) {
    const allContents = [];
    let continuationToken = null;
    let pageCount = 0;
    const startTime = Date.now();
    
    console.log(`[listS3Files] Starting to list files from s3://${bucketName}/${prefix}`);
    ...
    do {
        ...
        console.log(`[listS3Files] Page ${pageCount}: Found ${data.Contents ? data.Contents.length : 0} items. Total so far: ${allContents.length}`);
