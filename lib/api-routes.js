...
async function queryInfluxDB(sqlQuery) {
    return new Promise((resolve, reject) => {
        ...
        const options = {
            method: 'POST',
            headers: headers,
            agent: new (tsdbConfig.url.startsWith('https://') ? https : http).Agent({
                keepAlive: true,
                keepAliveMsecs: 1000, // Close idle connections after 1 second
                maxSockets: 5,        // Reduce max concurrent sockets
                maxFreeSockets: 1,    // Minimize idle connections
                timeout: 10000        // Reduce request timeout to 10 seconds
            })
        };
...
function setupApiRoutes(app, s3, readBucket, writeBucket, getInMemoryState, cache = {}, positionCache = null, tsdbWriteCount = 0, opts = {}) {
    ...
    app.get('/api/positions', async (req, res) => {
        try {
            const hours = parseFloat(req.query.hours || '24');
            const source = req.query.source || 'memory'; // memory, sqlite, tsdb, s3
            ...
            switch (source) {
                ...
                case 's3':
                    // Load from S3 historical data
                    const recentCutoff = Date.now() - (hours * 60 * 60 * 1000);
                    positions = (await loadRecentHeatmapPositions(s3, readBucket, writeBucket, recentCutoff)) || [];
                    break;
