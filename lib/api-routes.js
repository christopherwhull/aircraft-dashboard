...
const NODE_ENV = (config && config.server && config.server.env) || 'production';
const EXPECTED_RESTART_TOKEN = (config && config.server && config.server.restartToken) || '';
...
module.exports = {
    setupApiRoutes: (app, s3, readBucket, writeBucket, getInMemoryState, globalCache, positionCache) => {
        // --- API Routes ---

        // Route to serve the application logo
...
// Allow tests and environments to override git status via config (preferred), then env
const GIT_COMMIT_OVERRIDE = (config && config.server && config.server.gitCommitOverride) || '';
const GIT_DIRTY_OVERRIDE = (config && config.server && config.server.gitDirtyOverride) || false;
if (GIT_COMMIT_OVERRIDE) {
    serverGitCommit = GIT_COMMIT_OVERRIDE;
    serverGitDirty = !!GIT_DIRTY_OVERRIDE;
...
function setupApiRoutes(app, s3, readBucket, writeBucket, getInMemoryState, cache = {}, positionCache = null) {
    ...
    app.post('/api/restart', (req, res) => {
        try {
            if (!EXPECTED_RESTART_TOKEN) {
                logger.warn('[restart] Restart endpoint disabled: no restart token configured');
            ...
            if (providedToken !== EXPECTED_RESTART_TOKEN) {
    ...
    app.get('/api/health', async (req, res) => {
        try {
            // Check PiAware connectivity
            const piAwareUrl = config.dataSource.piAwareUrl;
            let piAwareConnected = false;
            ...
            if (piAwareUrl) {
                try {
                    ...
                    await new Promise((resolve, reject) => {
                        const parsedUrl = url.parse(piAwareUrl);
                        const options = {
                        ...
                        const req = http.request(options, (response) => {
                            piAwareConnected = response.statusCode === 200;
                            resolve();
                        });

                        req.on('error', () => {
                            piAwareConnected = false;
                            resolve();
                        });
            ...
            res.json({
                ...
                piAwareConnected: piAwareConnected
            });
        } catch (error) {
            res.status(500).json({
                status: 'error',
