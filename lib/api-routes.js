...
async function queryInfluxDB(sqlQuery) {
    return new Promise((resolve, reject) => {
        ...
        const httpModule = tsdbConfig.url.startsWith('https://') ? https : http;
...
async function loadAllHeatmapPositions(s3, readBucket, writeBucket) {
    ...
    const hourlyFilesRead = await listS3Files(s3, readBucket, 'data/hourly/positions_');
    const hourlyFilesWrite = await listS3Files(s3, writeBucket, 'hourly_');
    const files = [...minuteFiles, ...hourlyFilesRead, ...hourlyFilesWrite];
    console.log(`Found ${minuteFiles.length} minute files, ${hourlyFilesRead.length} legacy hourly files, and ${hourlyFilesWrite.length} consolidated hourly files. Total: ${files.length} files.`);
...
async function queryHeatmapFromTSDB(hours = 24, gridSizeNm = 1) {
    try {
        ...
        return new Promise((resolve, reject) => {
            const protocol = tsdbConfig.url.startsWith('https:') ? https : http;
...
function setupApiRoutes(app, s3, readBucket, writeBucket, getInMemoryState, cache = {}, positionCache = null, opts = {}) {
    
    // --- Heatmap Data Endpoint ---
    app.get('/api/heatmap-data', async (req, res) => {
        try {
            const source = req.query.source || 'memory'; // 'memory' or 'tsdb'
            ...
            if (source === 'tsdb') {
                ...
                gridData = tsdbResult || [];
                console.log(`[heatmap] TSDB returned ${gridData.length} grid cells`);
            } else {
    ...
    app.get('/api/squawk-transitions', async (req, res) => {
        try {
            ...
            if (req.query.startTime && req.query.endTime) {
                ...
                const transitionsData = await computeSquawkTransitionsDataByTimeRange(s3, writeBucket, startTime, endTime, 'both');
                
                // For each transition, find the position data from the same time period
                const transitionsWithPositions = [];
                
                for (const transition of transitionsData.transitions) {
                    // Look for position data for this aircraft around the transition time
                    // We'll search in a 5-minute window around the transition timestamp
                    const searchStart = transition.timestamp - (5 * 60 * 1000);
                    const searchEnd = transition.timestamp + (5 * 60 * 1000);
                    
                    // Get position data for this aircraft in the time window
                    const positions = await getAircraftPositionsInTimeRange(
                        s3, writeBucket, transition.hex, searchStart, searchEnd
                    );
                    
                    // Find the position closest to the transition timestamp
                    let closestPosition = null;
                    let minTimeDiff = Infinity;
                    
                    for (const pos of positions) {
                        const timeDiff = Math.abs(pos.timestamp - transition.timestamp);
                    ...
                    if (closestPosition) {
                        transitionsWithPositions.push({
                            ...transition,
                            lat: closestPosition.lat,
                            lon: closestPosition.lon,
                            positionTimestamp: closestPosition.timestamp,
                            timeDiff: Math.round(minTimeDiff / 1000) // seconds difference
                        });
                    } else {
                        // If no position found, include transition without position data
                        transitionsWithPositions.push(transition);
                    }
                }
                
                const data = {
                    transitions: transitionsWithPositions,
                    totalTransitions: transitionsWithPositions.length
                };
                ...
                // Use writeBucket since that's where current aircraft log files are stored
                const transitionsData = await computeSquawkTransitionsData(s3, writeBucket, hours, 'both');
    ...
    app.get('/api/heatmap-stats', async (req, res) => {
        try {
            const allPositions = await loadAllHeatmapPositions(s3, readBucket, writeBucket);
    ...
    app.get('/api/positions', async (req, res) => {
        try {
            ...
            if (!positions || positions.length === 0) {
                ...
                positions = (await loadAllHeatmapPositions(s3, readBucket, writeBucket)) || [];
            }
