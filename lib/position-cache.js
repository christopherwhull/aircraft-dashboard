...
class PositionCache {
    constructor(s3, buckets = {}, options = {}) {
        ...
        // Retention
        this.retentionMs = 10 * 365 * 24 * 60 * 60 * 1000; // 10 years
    ...
    async loadAllPositions() {
        ...
        try {
            ...
            // Load from the write bucket only
            const bucket = this.writeBucket;
            try {
                // Load minute files
                const s3Files = await listS3Files(this.s3, bucket, 'data/piaware_aircraft_log');
