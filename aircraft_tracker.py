...
registration_lookup = importlib.import_module('registration-lookup')
airline_lookup = importlib.import_module('airline-lookup')

# Extract functions from the imported modules
registration_from_hexid = registration_lookup.registration_from_hexid
get_airline_from_callsign = airline_lookup.get_airline_from_callsign

try:
	config_reader = importlib.import_module('config-reader')
...
# OpenSky ICAO database cache
opensky_db_cache: Optional[dict] = None  # Cached OpenSky aircraft database
opensky_db_loaded: bool = False  # Whether the database has been loaded
...
def load_opensky_database_from_s3() -> Optional[dict]:
	"""Load OpenSky aircraft database from S3 cache."""
	global opensky_db_cache, opensky_db_loaded
	
	if opensky_db_loaded and opensky_db_cache is not None:
		return opensky_db_cache
	
	if not BOTO3_AVAILABLE:
		print("Warning: boto3 not available, cannot load OpenSky database from S3")
		return None
	...
	try:
		...
		if s3_client is None:
			if CONFIG_READER_AVAILABLE:
				try:
					config = get_config()
					endpoint = config.get('s3Endpoint', 'http://localhost:9000')
					access_key = config.get('s3AccessKeyId', 'minioadmin')
					secret_key = config.get('s3SecretAccessKey', 'minioadmin123')
				except Exception:
		...
		obj_data = s3_client.get_object(Bucket='aircraft-data', Key='opensky_aircraft_cache.json')
		content = obj_data['Body'].read().decode('utf-8')
		data = json.loads(content)
		
		opensky_db_cache = data.get('aircraft', {})
		opensky_db_loaded = True
		
		print(f"✅ Loaded OpenSky ICAO database from S3: {len(opensky_db_cache)} aircraft records")
		return opensky_db_cache
		
	except Exception as e:
		print(f"❌ Error loading OpenSky database from S3: {e}")
		return None


def get_opensky_aircraft_info(hex_code: str) -> Optional[dict]:
	"""Get aircraft information from OpenSky database."""
	db = load_opensky_database_from_s3()
	if db and hex_code in db:
		return db[hex_code]
	return None
...
def update_aircraft_tracking(current_aircraft: List[dict]) -> None:
	...
	for aircraft in current_aircraft:
		...
		if hex_code not in aircraft_tracking:
			...
			# If still missing, try OpenSky ICAO database
			if registration in (None, 'N/A', '') or aircraft_type in (None, 'N/A', ''):
				opensky_info = get_opensky_aircraft_info(hex_code)
				if opensky_info:
					if registration in (None, 'N/A', ''):
						registration = opensky_info.get('registration', registration)
					if aircraft_type in (None, 'N/A', ''):
						aircraft_type = opensky_info.get('typecode', aircraft_type)
			
			# If still missing, lookup from static database
			if registration in (None, 'N/A', '') or aircraft_type in (None, 'N/A', ''):
				db_info = get_db_info(hex_code)
				if registration in (None, 'N/A', '') and 'r' in db_info:
					registration = db_info['r']
				if aircraft_type in (None, 'N/A', '') and 't' in db_info:
					aircraft_type = db_info['t']
			
			# After all lookups, if we have a definitive registration or type, cache it to S3
			if registration not in (None, 'N/A', '') or aircraft_type not in (None, 'N/A', ''):
				set_icao_cache_to_s3(hex_code, registration, aircraft_type)
			
			# Get additional aircraft information from OpenSky database
			manufacturer = None
			model = None
			operator = None
			owner = None
			...
			opensky_info = get_opensky_aircraft_info(hex_code)
			if opensky_info:
				manufacturer = opensky_info.get('manufacturername')
				model = opensky_info.get('model')
				operator = opensky_info.get('operator')
				owner = opensky_info.get('owner')
			...
			aircraft_tracking[hex_code] = {
				...
				'hex': hex_code,
				'flight': flight,
				'registration': registration if registration not in (None, 'N/A', '') else None,
				'type': aircraft_type if aircraft_type not in (None, 'N/A', '') else None,
			...
			# Enhance existing aircraft with OpenSky data if not already present
			if (aircraft_tracking[hex_code].get('manufacturer') is None or
				aircraft_tracking[hex_code].get('model') is None or
				aircraft_tracking[hex_code].get('operator') is None or
				aircraft_tracking[hex_code].get('owner') is None):
				
				opensky_info = get_opensky_aircraft_info(hex_code)
				if opensky_info:
					if aircraft_tracking[hex_code].get('manufacturer') is None:
						aircraft_tracking[hex_code]['manufacturer'] = opensky_info.get('manufacturername')
					if aircraft_tracking[hex_code].get('model') is None:
						aircraft_tracking[hex_code]['model'] = opensky_info.get('model')
					if aircraft_tracking[hex_code].get('operator') is None:
						aircraft_tracking[hex_code]['operator'] = opensky_info.get('operator')
					if aircraft_tracking[hex_code].get('owner') is None:
						aircraft_tracking[hex_code]['owner'] = opensky_info.get('owner')
